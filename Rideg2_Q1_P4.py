...
n,d = X_tr.shape
X_tr = X_tr - np.dot(np.ones([n,1]).T,X_tr)/n #Making zero mean
y_tr = y_tr - np.dot(np.ones([n,1]).T,y_tr)/n #Making zero mean
...
nt,dt = X_te.shape
X_te = X_te - np.dot(np.ones([nt,1]).T,X_te)/nt #Making zero mean
y_te = y_te - np.dot(np.ones([nt,1]).T,y_te)/nt #Making zero mean
